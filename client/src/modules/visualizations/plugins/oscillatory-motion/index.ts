  ...
  createInstance(): VisualizationInstance {
    ...
    return {
      ...
      render(ctx: VisualizationRenderContext, audio: AudioFrameData, params: Record<string, unknown>) {
        ...
        const phaseJitterSpeed = treble * 0.02 * baseSpeed;
        const oscillatorBlend = energy;

        for (const osc of oscillators) {
          const basePhaseSpeed = 0.02 * baseSpeed;
          osc.phaseVelocity = basePhaseSpeed + phaseJitterSpeed;
          osc.phase1 += osc.phaseVelocity;
          osc.phase2 += osc.phaseVelocity * 1.5;
        }

        time += 0.016 * baseSpeed;

        if (showTrails) {
          const fadeSpeed = 0.05 + energy * 0.1 + (isBeat ? 0.15 : 0);
          context.fillStyle = `rgba(0, 0, 0, ${clamp(fadeSpeed, 0, 1)})`;
        ...
        if (isBeat) {
          ...
          context.fillStyle = `hsla(${flashHue}, 80%, 50%, 0.1)`;
          context.fillRect(0, 0, width, height);
        }
        ...
        if (mode === 'rings') {
          for (let ring = 0; ring < ringCount; ring++) {
            ...
            const osc1 = Math.sin(omega * time + osc.phase1 + ringPhase);
            const osc2 = Math.sin(omega * 1.5 * time + osc.phase2 + ringPhase);
            const combinedOsc = lerp(osc1, osc1 + 0.5 * osc2, oscillatorBlend);
            
            const radiusOsc = baseRadius + combinedOsc * amplitude * (0.5 + ring * 0.1);

            context.beginPath();
            for (let i = 0; i <= pointCount; i++) {
              const theta = (i / pointCount) * Math.PI * 2;
              const pointOsc = Math.sin(omega * time + theta * 3 + osc.phase1);
              const r = radiusOsc + pointOsc * amplitude * 0.3;
          ...
          for (let i = 0; i < particles.length; i++) {
            ...
            const osc1 = Math.sin(omega * time + p.phase + osc.phase1);
            const osc2 = Math.sin(omega * 1.5 * time + p.phase + osc.phase2);
            const combinedOsc = lerp(osc1, osc1 + 0.5 * osc2, oscillatorBlend);
            
            const x = p.baseX + combinedOsc * amplitude;
            const y = p.baseY + Math.cos(omega * time + p.phase + osc.phase1) * amplitude;
            ...
            const size = p.size * (1 + instantBass * 2);
            ...
            if (instantBass > 0.3) {
              ...
              context.strokeStyle = `hsla(${hue}, ${saturation}%, ${lightness}%, ${alpha * 0.3})`;
              context.lineWidth = 1;
              context.stroke();
            }
          }
          ...
          const a = Math.round(map(bass, 0, 1, 1, 5));
          const b = Math.round(map(mid, 0, 1, 2, 7));
          ...
          for (let layer = 0; layer < 3; layer++) {
            ...
            for (let i = 0; i <= pointCount * 4; i++) {
              ...
              const x = cx + A * layerScale * Math.sin(a * t + delta + layerPhase);
              const y = cy + B * layerScale * Math.sin(b * t + layerPhase);
        ...
        if (instantBass > 0.5) {
          context.save();
          const glowRadius = 100 + instantBass * 150;
          const gradient = context.createRadialGradient(cx, cy, 0, cx, cy, glowRadius);
          const hue = getColorHue(colorMode, time * 30, energy, treble, colorSensitivity);
          gradient.addColorStop(0, `hsla(${hue}, 80%, 60%, ${instantBass * 0.3})`);
